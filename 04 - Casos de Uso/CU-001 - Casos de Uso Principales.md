...
- Flujo principal: Trader cancela la [[Order]] antes de que se ejecute.
...
- Flujo principal: Trader decide cerrar la [[Position]] antes de que la [[Strategy]] lo haga automáticamente (ver PR-07).
...
- Flujo principal: Trader cambia `maxRiskPerTrade`, `stopLoss`, `takeProfit` o `maxExposure` del [[Portfolio]]. Se reemplaza el Value Object completo (DOM-004/DOM-005).
...
## PR-01 Analizar mercado y generar [[Signal]]
...
## PR-02 Validar riesgo de una [[Signal]]
...
- Flujo principal: se envía la [[Order]] al [[Exchange]] configurado vía el Port/Adapter correspondiente (ADR-004).
- Postcondición: `OrderSubmitted`. La confirmación de ejecución llega después, de forma asíncrona, desde el [[Exchange]].
- Alternos: si el [[Exchange]] rechaza la [[Order]] al enviarla, `OrderRejected`.
...
| 1.1     | 17/07/2026 | Miler Ayarza | Se agregaron wikilinks [[...]] faltantes en Strategy, Order, Position, Portfolio, Signal y Exchange, para mantener consistencia con DOM-001 a DOM-007 |
